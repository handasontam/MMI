...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model, index=0):

        score = -val_loss

        if self.best_score is None:
            self.best_score = score
            self.save_checkpoint(val_loss, model, index)
    ...
    def save_checkpoint(self, val_loss, model, index=0):
        ...
        torch.save(model.state_dict(), ch)# 'checkpoint.pt')
