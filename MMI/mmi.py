    ...
    def condEntTables2subsetEntTables(self, condEntTables, verbose=False):
        ...
        if self.log == True:
            if np.ndarray == type(condEntTables):
                ...
                for model_name in condEntTables.keys():
                    np.savetxt("{0}condEntropyTable_{1}.txt".format(self.prefix, model_name), condEntTables[model_name])
                    np.savetxt("{0}subsetEntropyTable_{1}.txt".format(self.prefix, model_name), subsetEnt[model_name])
        return subsetEnt
...
def get_estimation_MMI(data_model, varying_param, prefix=""):
    ...
    mmi = MMI(data.shape[1])
    subsetEntTable = mmi.condEntTables2subsetEntTables(condEntTables)

    for model_name, model in MMI_config.model.items():
        mmi_estimation = mmi.computeMMI(subsetEntTable[model_name])
        # Save Results
        results[model_name] = mmi_estimation
...
def plotMMI_fromSetting():
    ...
    for data_name, data in tqdm(MMI_config.data.items()):
        ...
        for var in MMI_config.variables:
            prefix_name_loop = "{0}n_var={1}/".format(prefix_name, var)
